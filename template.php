...
function kada_preprocess_page(&$variables) {
  $path = backdrop_get_path('theme', 'kada');
  // Font stylesheets according to theme settings.
  if (theme_get_setting('font') == 'stix') {
    backdrop_add_css($path . '/css/stix.css');
  }
  elseif (theme_get_setting('font') == 'merriweather') {
 ...
 * @see layout.tpl.php
 */
function kada_preprocess_layout(&$variables) {
 ...
 */
function kada_js_alter(&$javascript) {
 ...
 */
function kada_ckeditor_settings_alter(&$settings, $format) {
  global $base_url, $base_path;
  $path = $base_path . backdrop_get_path('theme', 'kada');
  $font_selected = theme_get_setting('font');

  if ($font_selected == 'stix') {
    $settings['contentsCss'][] = $path . '/css/stix.css';
  }
  elseif ($font_selected == 'merriweather') {
...
function kada_preprocess_header(&$variables) {
  ...
  $variables['logo_includes_site_name'] = theme_get_setting('logo_includes_site_name', 'kada');
}
